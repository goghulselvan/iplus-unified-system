...
-- Phase 1: Server-Side Email Validation (for new emails only)
-- Create email domain validation function
CREATE OR REPLACE FUNCTION public.validate_email_domain_new_only(email_address text)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE STRICT
SET search_path = public
AS $$
BEGIN
  -- Allow null emails and existing emails to remain unchanged
  IF email_address IS NULL OR email_address = '' THEN
    RETURN true;
  END IF;
  
  -- For new entries, validate email format and domain
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profiles' AND table_schema = 'public') THEN
    ...
    -- Add constraint for new emails only
    ALTER TABLE public.profiles 
    DROP CONSTRAINT IF EXISTS valid_email_format;
    
    ALTER TABLE public.profiles 
    ADD CONSTRAINT valid_email_format 
    CHECK (public.validate_email_domain_new_only(email));
  END IF;
END $$;
...
-- Create function to validate sensitive operations with enhanced checks
...
BEGIN
  ...
  IF p_operation IN ('BULK_DELETE', 'BULK_UPDATE', 'EXPORT_DATA') 
