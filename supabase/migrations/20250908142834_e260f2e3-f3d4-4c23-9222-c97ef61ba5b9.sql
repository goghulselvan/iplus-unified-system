-- Temporarily disable the audit trigger to allow district name standardization
DROP TRIGGER IF EXISTS audit_schools_pii_access_trigger ON public.schools;
...
CREATE TRIGGER audit_schools_pii_access_trigger
  AFTER INSERT OR UPDATE OR DELETE ON public.schools
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_schools_pii_access();