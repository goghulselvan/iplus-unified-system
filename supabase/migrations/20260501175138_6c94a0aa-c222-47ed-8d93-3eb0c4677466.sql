-- 1. Backfill schools.total_participants from actual student_subjects counts
UPDATE public.schools s
SET total_participants = sub.cnt, updated_at = now()
FROM (
  SELECT sr.school_id, COUNT(ss.id) AS cnt
...
WHERE s.id = sub.school_id
  AND (s.total_participants IS DISTINCT FROM sub.cnt);

-- 2. Backfill school_project_workflow.total_participants for the school's current project
UPDATE public.school_project_workflow w
SET total_participants = s.total_participants, updated_at = now()
FROM public.schools s
WHERE w.school_id = s.id
  AND w.project_id = s.current_project_id
  AND (w.total_participants IS DISTINCT FROM s.total_participants);

-- 3. Update update_total_participants() to also persist count to school_project_workflow
...
BEGIN
  ...
  IF affected_school_id IS NOT NULL THEN
    ...
    -- Mirror into the workflow row so project switch / mirror does not wipe it
    SELECT current_project_id INTO v_current_project FROM public.schools WHERE id = affected_school_id;
    IF v_current_project IS NOT NULL THEN
      UPDATE public.school_project_workflow
         SET total_participants = participation_count, updated_at = now()
       WHERE school_id = affected_school_id AND project_id = v_current_project;
    END IF;
  END IF;
...
-- 4. Patch _mirror_workflow_to_school to never overwrite a known live count with NULL
...
BEGIN
  ...
  ELSE
    UPDATE public.schools SET
      ...
      -- Preserve live count when workflow row has none
      total_participants=COALESCE(w.total_participants, schools.total_participants),
      updated_at=now()
    WHERE id = p_school_id;
  END IF;
...
-- 5. Patch rehydrate_all_schools_for_project the same way
...
BEGIN
  ...
  UPDATE public.schools s SET
    ...
    per_entry_rate=COALESCE(w.per_entry_rate,150),
    concession_per_entry=COALESCE(w.concession_per_entry,0),
    total_participants=COALESCE(w.total_participants, s.total_participants),
    updated_at=now()
