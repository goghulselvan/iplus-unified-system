-- Update the dashboard metrics function to include consent form sent metrics
...
-- Create a new function for date-filtered metrics
CREATE OR REPLACE FUNCTION public.get_dashboard_metrics_by_date(target_date date)
 RETURNS TABLE(total_schools bigint, courier_sent bigint, courier_returned bigint, contacted_yes bigint, contacted_no bigint, registration_interested bigint, registration_not_interested bigint, consent_requested bigint, consent_form_sent_total bigint, consent_form_sent_physical bigint, consent_form_sent_digital bigint, registration_confirmed bigint, name_list_received bigint, payment_received bigint, question_paper_sent bigint, answer_sheet_received bigint, result_sent bigint, communications_count bigint, follow_ups_created bigint, follow_ups_completed bigint)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
