...
-- This ensures it bypasses any potential PostgREST limits
...
AS $$
  WITH registration_count AS (
    -- Explicit count to ensure we get ALL registrations
  ...
  SELECT 
    ...
    COALESCE(rc.reg_count, 0) as total_registrations
  FROM public.schools s
  CROSS JOIN registration_count rc
