...
-- included address1/address2 — the SAME bug class as the portal_registered
-- fix in 20260718_search_schools_portal_registered.sql. Every searched or
-- default-sorted school list goes through this RPC (useSchoolsPaginated ->
-- fetchSchools), so address1/address2 always came back undefined there,
-- even though School Detail and a raw select('*') show them correctly.
-- SchoolCard's address1||address2||school_address fallback always fell
-- through to the stale school_address as a result. Return type changes, so
