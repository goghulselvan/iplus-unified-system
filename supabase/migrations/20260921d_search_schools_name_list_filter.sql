-- The Overview cross-tab click was setting real React state (confirmed by the
-- banner rendering correctly) but the actual list never filtered. Root cause,
-- found by finally reading the real fetch path: applyFilters -> fetchSchools
-- calls search_schools_case_insensitive, a server-side RPC with a fixed
-- parameter list — buildQuery (everything I'd wired crosstabFilter into) is
-- separate, dead code never called by the real listing flow at all.
--
-- status_filter and payment_filter already do exact-value matching, reused
-- directly for the crosstab's registration/payment fields. There was no way
-- to filter by name_list_status as an exact value at all — workflow_filter
-- only offers it as two mutually-exclusive named options, not combinable with
-- the other two fields in one query. Adding name_list_filter, using the
-- COALESCE(workflow, schools) pattern the rest of this RPC's workflow_filter
-- branch already uses — status_filter/payment_filter read the raw schools
-- column only, a pre-existing inconsistency left alone here, out of scope
-- for this fix.
-- CREATE OR REPLACE doesn't merge signatures with a different arg count —
-- it creates an overload instead, and callers matching only by named
-- parameters then fail as ambiguous. Drop the old 10-arg signature first.
DROP FUNCTION IF EXISTS public.search_schools_case_insensitive(
...
CREATE FUNCTION public.search_schools_case_insensitive(
  ...
  name_list_filter text DEFAULT NULL
)
RETURNS TABLE(
  id uuid, ss_no integer, school_name text, school_address text, address1 text, address2 text,
  district text, state text, board text, pincode text, contact_person_name text, email text,
  mobile1 text, mobile2 text, courier_status courier_status, contacted contacted_status,
  registration_interest interest_status, registration_interest_comment text,
  consent_form_requested consent_status, consent_form_comment text, consent_form_sent text,
  registration_status registration_status, name_list_status name_list_status, payment_status payment_status,
  payment_date date, payment_amount numeric, payment_mode text,
  question_paper_sent question_paper_status, answer_sheet_status answer_sheet_status, result_status result_status,
  total_participants integer, brochure_delivery_status brochure_delivery_status,
  current_project_id uuid, created_at timestamptz, updated_at timestamptz,
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (name_list_filter IS NULL OR COALESCE(w.name_list_status::text, s.name_list_status::text) = name_list_filter)
      AND (workflow_filter IS NULL OR (
        ...
        (workflow_filter = 'payment_received'          AND COALESCE(w.payment_status::text,          s.payment_status::text)           IN ('Received', 'Overpaid')) OR
