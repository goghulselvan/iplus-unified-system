-- Prospect schools: labeled contact numbers (Principal / School / Coordinator / etc.),
-- up to 5, same [{name, mobile, role}] jsonb shape as schools.additional_contacts.
-- Also returned by get_prospect_schools so the side panel + e-brochure dialog can use them.

ALTER TABLE public.prospect_schools
...
BEGIN
  ...
  SELECT json_agg(t) INTO v_rows FROM (
    SELECT
      ...
      ps.linked_to_crm, ps.is_active, ps.additional_contacts,
      CASE
