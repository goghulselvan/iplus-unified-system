-- Update correct_student_registration to wait for registration number generation
...
BEGIN
  ...
  -- Wait for registration number to be generated by trigger (with retry)
  WHILE v_retry_count < 10 AND v_new_registration_number IS NULL LOOP
    SELECT registration_number_generated
    INTO v_new_registration_number
    FROM student_registrations
    WHERE id = v_new_registration_id;
    
    IF v_new_registration_number IS NULL THEN
      PERFORM pg_sleep(0.1); -- Wait 100ms
      v_retry_count := v_retry_count + 1;
    END IF;
  END LOOP;

  -- If still NULL after retries, generate it manually
  IF v_new_registration_number IS NULL THEN
    -- Get the first subject for generation
    SELECT subject_id INTO v_old_subjects[1]
    FROM student_subjects
    WHERE registration_id = v_new_registration_id
    LIMIT 1;
    
    IF v_old_subjects[1] IS NOT NULL THEN
      v_new_registration_number := generate_registration_number(
        v_old_registration.school_id,
        v_old_registration.project_id,
        COALESCE(p_new_class, v_old_registration.student_class),
        v_old_subjects[1]
      ...
      SET registration_number_generated = v_new_registration_number
      WHERE id = v_new_registration_id;
    END IF;
  END IF;
