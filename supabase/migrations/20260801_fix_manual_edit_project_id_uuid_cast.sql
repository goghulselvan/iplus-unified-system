-- update_school_with_manual_edit crashed with "invalid input syntax for type
-- uuid" whenever a manual edit payload included current_project_id as an
-- empty string. Every other nullable field in this function guards against
-- empty string via NULLIF(...,'') before casting/assigning; current_project_id
-- was the one field that cast (p_updates->>'current_project_id')::uuid
-- directly with no guard, so ''::uuid crashed the whole edit -- confirmed by
-- reproducing it exactly for SS 12655 (VIJAYALAKSHMI PUBLIC SCHOOL).
-- Fix mirrors the same NULLIF pattern already used elsewhere in this
-- function; COALESCE semantics unchanged (empty/missing = leave as-is).
...
BEGIN
  ...
  UPDATE public.schools SET
    ...
    current_project_id        = COALESCE(NULLIF(p_updates->>'current_project_id', '')::uuid, current_project_id),
    updated_at                = now()
