...
-- Insert existing boards from schools table with proper code generation to avoid duplicates
...
SELECT DISTINCT 
  ...
  UPPER(REPLACE(REPLACE(REPLACE(TRIM(board), ' ', '_'), '&', 'AND'), '-', '_')) || '_' || 
  ROW_NUMBER() OVER (PARTITION BY UPPER(REPLACE(REPLACE(REPLACE(TRIM(board), ' ', '_'), '&', 'AND'), '-', '_')) ORDER BY TRIM(board)) as board_code
FROM public.schools 
WHERE board IS NOT NULL AND TRIM(board) != ''
ON CONFLICT (board_name) DO NOTHING;

-- Update board codes to be unique for existing entries
UPDATE public.boards 
SET board_code = UPPER(REPLACE(REPLACE(REPLACE(board_name, ' ', '_'), '&', 'AND'), '-', '_')) || '_' || 
  (SELECT ROW_NUMBER() OVER (ORDER BY created_at) FROM public.boards b2 WHERE b2.board_name = boards.board_name)
WHERE board_code IS NULL OR board_code = '';
