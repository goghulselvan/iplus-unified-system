...
-- Handle existing Tamil Nadu entry
UPDATE public.state_codes SET state_code = '33' WHERE UPPER(TRIM(state_name)) = 'TAMIL NADU';

-- Insert all other states (skip if already exists)
DO $$
BEGIN
  -- Insert state codes only if they don't exist
  ...
  SELECT state_name, state_code FROM (VALUES
    ...
    ('Lakshadweep', '31'),
    ('Puducherry', '34')
  ) AS new_states(state_name, state_code)
  WHERE NOT EXISTS (
    SELECT 1 FROM public.state_codes sc 
    WHERE UPPER(TRIM(sc.state_name)) = UPPER(TRIM(new_states.state_name))
  );
END $$;