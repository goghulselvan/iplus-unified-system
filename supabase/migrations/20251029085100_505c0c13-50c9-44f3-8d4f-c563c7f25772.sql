-- Drop and recreate get_dashboard_metrics_by_project to include total_registrations
DROP FUNCTION IF EXISTS public.get_dashboard_metrics_by_project(uuid);

CREATE FUNCTION public.get_dashboard_metrics_by_project(p_project_id uuid DEFAULT NULL)
RETURNS TABLE(
...
AS $$
  SELECT 
    ...
    (SELECT COUNT(*) FROM public.student_registrations sr WHERE (p_project_id IS NULL OR sr.project_id = p_project_id)) as total_registrations
  FROM public.schools s
  WHERE (p_project_id IS NULL OR s.current_project_id = p_project_id);
$$;