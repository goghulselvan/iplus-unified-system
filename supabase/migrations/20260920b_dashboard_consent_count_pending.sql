-- New Dashboard metric: schools that clicked "Consent Form Requested: Yes" but
-- have no real count entered yet in the Consent Form tab. This is the actual
-- follow-up list ("go get the number from these schools") that the flat
-- consent_requested tile couldn't show — it only ever counted the flag, never
-- whether a real number backs it.
-- CREATE OR REPLACE can't change a RETURNS TABLE column list — drop first.
DROP FUNCTION IF EXISTS public.get_dashboard_metrics_by_project_with_access(uuid);

CREATE FUNCTION public.get_dashboard_metrics_by_project_with_access(p_project_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(
...
BEGIN
  ...
  WITH accessible_schools AS (
    SELECT
      ...
      -- Real count for the SAME project this row is scoped to. When
      -- p_project_id is NULL ("all projects" view) this can't mean any one
      -- project's count, so it's left NULL and consent_count_pending reads 0
      -- in that mode — same documented limitation as every other
      -- project-scoped tile on this page.
