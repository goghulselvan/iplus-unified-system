-- Check current district codes for Tamil Nadu and fix conflicts
-- First, let's see what we have and clean up step by step

-- Step 1: Remove any existing Tamil Nadu entries for codes 011 and 018 (duplicates)
DELETE FROM public.district_codes 
WHERE state_code = '33' AND district_code IN ('011', '018');

-- Step 2: Update KANNIYAKUMARI (011) and NILGIRIS (018) from West Bengal to Tamil Nadu
...
-- Step 3: Remove duplicate entries in Tamil Nadu (codes 039 and 040)
DELETE FROM public.district_codes 
WHERE state_code = '33' AND district_code IN ('039', '040');

-- Step 4: Remove any other incorrect Nilgiris duplicates in Tamil Nadu
DELETE FROM public.district_codes 
WHERE state_code = '33' 
  AND district_name IN ('Nilgiris', 'THE NILGIRIS') 
  AND district_code NOT IN ('011', '018');

-- Step 5: Clear school codes for affected schools (they'll be reassigned with correct district codes)
...
-- Step 6: Reset student registration sequences (fresh start with new numbering)
DELETE FROM public.student_registration_sequences;

-- Step 7: Update the registration number generation function to new format
