...
-- Remove subject_id from student_registrations as it's now in junction table
ALTER TABLE public.student_registrations DROP COLUMN IF EXISTS subject_id;

-- Add trigger for student_subjects audit
...
BEGIN
  ...
  LOOP
    -- Insert the main registration record
  ...
  -- Delete old records that had subject_id
