-- Update get_dashboard_metrics_by_project to use stored total_participants
-- This accounts for corrections and retired registrations
...
BEGIN
  ...
  SELECT
    ...
    -- Use stored total_participants which accounts for corrections and retired registrations
    COALESCE(SUM(s.total_participants) FILTER (WHERE p_project_id IS NULL OR s.current_project_id = p_project_id), 0) as total_registrations
  FROM schools s;
END;
