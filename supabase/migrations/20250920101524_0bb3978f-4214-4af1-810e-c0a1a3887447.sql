-- Create the missing dashboard metrics function that includes schools with NULL project IDs
CREATE OR REPLACE FUNCTION public.get_dashboard_metrics_by_project(p_project_id uuid DEFAULT NULL)
RETURNS TABLE(
...
AS $$
  SELECT 
    ...
    COALESCE((
      ...
      WHERE (p_project_id IS NULL OR sch.current_project_id = p_project_id OR sch.current_project_id IS NULL)
    ), 0) as total_registrations
  FROM public.schools s
  WHERE (p_project_id IS NULL OR s.current_project_id = p_project_id OR s.current_project_id IS NULL);
$$;